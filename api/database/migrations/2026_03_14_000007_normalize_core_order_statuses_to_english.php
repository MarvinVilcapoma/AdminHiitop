...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (Schema::hasColumn('order_statuses', 'is_protected')) {
            ...
            DB::table('order_statuses')
                ->where('slug', 'pagado')
                ->update(['is_protected' => false, 'updated_at' => $now]);
        }
    ...
    public function down(): void
    {
        ...
        DB::table('order_statuses')
            ->where('slug', 'pending')
            ->update(['slug' => 'pendiente', 'name' => 'Pendiente', 'updated_at' => $now]);

        DB::table('order_statuses')
            ->where('slug', 'cancelled')
            ->update(['slug' => 'cancelado', 'name' => 'Cancelado', 'updated_at' => $now]);

        DB::table('order_statuses')
            ->where('slug', 'delivered')
            ->update(['slug' => 'entregado', 'name' => 'Entregado', 'updated_at' => $now]);
    }
