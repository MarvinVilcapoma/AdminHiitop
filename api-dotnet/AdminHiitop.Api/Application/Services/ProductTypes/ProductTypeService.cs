...
public sealed class ProductTypeService : IProductTypeService
{
    ...
    public Task<ProductType?> GetByIdAsync(int id, CancellationToken cancellationToken)
        => _context.ProductTypes.AsNoTracking().Include(item => item.Sizes).FirstOrDefaultAsync(item => item.Id == id, cancellationToken);

    public async Task<ProductType> CreateAsync(ProductType request, CancellationToken cancellationToken)
    {
    ...
    public async Task<ProductType> UpdateAsync(int id, ProductType request, CancellationToken cancellationToken)
    {
        ProductType entity = await FindAsync(id, cancellationToken);
    ...
    public async Task<object> SyncSizesAsync(int productTypeId, SyncSizesRequest request, CancellationToken cancellationToken)
    {
        ProductType entity = await _context.ProductTypes.Include(item => item.Sizes)
            .FirstOrDefaultAsync(item => item.Id == productTypeId, cancellationToken)
    ...
    private async Task<ProductType> FindAsync(int id, CancellationToken cancellationToken)
    {
        ProductType? entity = await _context.ProductTypes.FirstOrDefaultAsync(item => item.Id == id, cancellationToken);
